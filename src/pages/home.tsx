...
import { getOwned, getShiny } from '../services/store/sections/pokedexState';
...
export const HomePage: Component = () => {
    ...
    const [selectedPokedexes, setSelectedPokedexes] = createSignal<Array<string>>([]);
    ...
    const getAllPokeDexData = async () => {
        ...
        try {
            ...
            setSelectedPokedexes(allPokeData.map(pd => pd.game))
        } catch (e) {
            setPokedexByRegion([]);
        }
    }
    ...
    return (
        <CommonLayout>
            ...
            <Show when={networkState() == NetworkState.Success}>
                <PageHeader text="Living dex lookup" />
                    ...
                    <Show when={mode() == LivingDexMode.latestGame}>
                        ...
                        <Flex id={latestGameDropdownId} flex={2} justifyContent="center" flexDirection="column">
                            <Dropdown
                                ...
                                multiple={true}
                                maxHeight="50vh"
                                onSelect={setSelectedPokedexes}
                                selectedValues={selectedPokedexes()}
                        ...
                        <For each={pokemonByGame()}>
                            {(pkmByGame) => (
                                <PokemonBox
                                    ...
                                    selectedPokedexes={selectedPokedexes()}
